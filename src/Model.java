...
class Model {
	...
	public static final float REST_RECHARGE_BONUS = 0.002f; // The amount of extra recharge if you are not moving or
															// throwing
	public static final float BROKEN_CRAWL_RATE = 0.3f; // The relative rate at which a broken robot can crawl back to
														// its flag, then reassenble itself
	...
	public static final float BOMB_DAMAGE_TO_FLAG = 0.05f; // The amount of energy a detonating bomb takes from a flag
															// within its blast radius
	public static final float BOMB_DAMAGE_TO_SPRITE = 0.3f; // The amount of energy a detonating bomb takes from all
															// sprites within the blast radius
	...
	private void checkSymbol(Object symbol) {
		if (symbol != this.secret_symbol)
			throw new NullPointerException("Counterfeit symbol!");
	}

	boolean amIblue(Object symbol) {
		checkSymbol(symbol);
		return sprites_self == sprites_blue;
	}

	void setPerspectiveBlue(Object symbol) {
		checkSymbol(symbol);
		sprites_self = sprites_blue;
		sprites_opponent = sprites_red;
	}

	void setPerspectiveRed(Object symbol) {
		checkSymbol(symbol);
		sprites_self = sprites_red;
		sprites_opponent = sprites_blue;
	}

	void setFlagEnergyBlue(Object symbol, float val) {
		checkSymbol(symbol);
		energy_blue = val;
	}

	void setFlagEnergyRed(Object symbol, float val) {
		checkSymbol(symbol);
		energy_red = val;
	}

	byte[] getTerrain(Object symbol) {
		checkSymbol(symbol);
		return this.terrain;
	}

	ArrayList<Sprite> getSpritesBlue(Object symbol) {
		checkSymbol(symbol);
		return this.sprites_blue;
	}

	ArrayList<Sprite> getSpritesRed(Object symbol) {
		checkSymbol(symbol);
		return this.sprites_red;
	}

	ArrayList<Bomb> getBombsFlying(Object symbol) {
		checkSymbol(symbol);
		return this.bombs_flying;
	}

	ArrayList<Bomb> getBombsExploding(Object symbol) {
		checkSymbol(symbol);
		return this.bombs_exploding;
	}
	...
	long getTimeBalance() {
		return controller.getTimeBalance(secret_symbol, sprites_self == sprites_blue);
	}

	float getFlagEnergySelf() {
		return (sprites_self == sprites_blue ? energy_blue : energy_red);
	}

	float getFlagEnergyOpponent() {
		return (sprites_self == sprites_blue ? energy_red : energy_blue);
	}

	int getSpriteCountSelf() {
		return sprites_self.size();
	}

	float getX(int sprite) {
		return sprites_self.get(sprite).x;
	}

	float getY(int sprite) {
		return sprites_self.get(sprite).y;
	}

	float getEnergySelf(int sprite) {
		return sprites_self.get(sprite).energy;
	}

	int getSpriteCountOpponent() {
		return sprites_opponent.size();
	}

	float getXOpponent(int opponent) {
		return XMAX - sprites_opponent.get(opponent).x;
	}

	float getYOpponent(int opponent) {
		return YMAX - sprites_opponent.get(opponent).y;
	}

	float getEnergyOpponent(int sprite) {
		return sprites_opponent.get(sprite).energy;
	}

	int getBombCount() {
		return bombs_flying.size();
	}
	...
	void setDestination(int sprite, float x, float y) {
		...
		if (s.energy >= 0) { // when you are dead, you cannot change your destination
			for (int i = 0; i < sprites_self.size(); i++) {
				...
				if (i != sprite && (x - t.xDestination) * (x - t.xDestination)
						+ (y - t.yDestination) * (y - t.yDestination) < 100) {
					x += 15;
					y += 10;
				}
			}
	...
	double getDistanceToDestination(int sprite) {
		Sprite s = sprites_self.get(sprite);
		return Math.sqrt(
				(s.x - s.xDestination) * (s.x - s.xDestination) + (s.y - s.yDestination) * (s.y - s.yDestination));
	}
	...
	class Sprite {
		...
		void update() {
			...
			if (energy < 0.0f) {
				...
				if ((x - Model.XFLAG) * (x - Model.XFLAG) + (y - Model.YFLAG) * (y - Model.YFLAG) < Model.BLAST_RADIUS
						* Model.BLAST_RADIUS)
					energy = 0.0f;
			} else
	...
	static class Bomb {
		...
		float getX() {
			return (Math.min(1.0f, (position / distance)) * (xEnd - xBegin)) + xBegin;
		}

		float getY() {
			return (Math.min(1.0f, (position / distance)) * (yEnd - yBegin)) + yBegin;
		}

		boolean isDetonating() {
			return prevPos < distance && position >= distance;
		}
	}
}
