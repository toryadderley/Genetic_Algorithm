...
class ReflexAgent implements IAgent {
	...
	void beFlagAttacker(Model m, int i) {
		...
		if (sq_dist(m.getX(i), m.getY(i), Model.XFLAG_OPPONENT, Model.YFLAG_OPPONENT) <= Model.MAX_THROW_RADIUS
	...
	void beAggressor(Model m, int i) {
		...
		if (index >= 0) {
			...
			if (m.getEnergySelf(i) >= m.getEnergyOpponent(index)) {
				...
				dx *= t;
				dy *= t;
				m.setDestination(i, enemyX + dx * (Model.MAX_THROW_RADIUS - Model.EPSILON),
						enemyY + dy * (Model.MAX_THROW_RADIUS - Model.EPSILON));
				...
				if (sq_dist(enemyX, enemyY, myX, myY) <= (Model.MAX_THROW_RADIUS + Model.BLAST_RADIUS)
						* (Model.MAX_THROW_RADIUS + Model.BLAST_RADIUS)) {
