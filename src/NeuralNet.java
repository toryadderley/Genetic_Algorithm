	...
	void computeError(double[] target) {
		if (target.length != activation.length)
			throw new IllegalArgumentException(
					"size mismatch. " + Integer.toString(target.length) + " != " + Integer.toString(activation.length));
...
class LayerLinear extends Layer {
	...
	double[] forwardProp(double[] in) {
		if (in.length != weights.rows())
			throw new IllegalArgumentException(
					"size mismatch. " + Integer.toString(in.length) + " != " + Integer.toString(weights.rows()));
		for (int i = 0; i < activation.length; i++)
			activation[i] = bias[i];
	...
	double[] forwardProp2(double[] in1, double[] in2) {
		if (in1.length + in2.length != weights.rows())
			throw new IllegalArgumentException("size mismatch. " + Integer.toString(in1.length) + " + "
					+ Integer.toString(in2.length) + " != " + Integer.toString(weights.rows()));
		for (int i = 0; i < activation.length; i++)
			activation[i] = bias[i];
...
class LayerTanh extends Layer {
	...
	double[] forwardProp(double[] in) {
		if (in.length != outputCount())
			throw new IllegalArgumentException(
					"size mismatch. " + Integer.toString(in.length) + " != " + Integer.toString(outputCount()));
		for (int i = 0; i < activation.length; i++) {
...
public class NeuralNet {
	...
	/// General-purpose constructor. (Starts with no layers. You must add at least
	/// one.)
	NeuralNet() {
	...
	/// Feeds "in" into this neural network and propagates it forward to compute
	/// predicted outputs.
	double[] forwardProp(double[] in) {
		for (int i = 0; i < layers.size(); i++) {
	...
	/// Feeds the concatenation of "in1" and "in2" into this neural network and
	/// propagates it forward to compute predicted outputs.
	double[] forwardProp2(double[] in1, double[] in2) {
	...
	/// Backpropagates the error from another neural network. (This is used when
	...
	/// Refines the weights and biases with on iteration of stochastic gradient
	/// descent.
