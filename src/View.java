...
public class View extends JFrame implements ActionListener {
	...
	class MyPanel extends JPanel {
		...
		MyPanel() throws Exception {
			...
			this.sound_doing = new MySoundClip("../audio/metal_doing.wav", 3);
		}
		...
		private void drawSprites(Graphics g) {
			...
			for (int i = 0; i < sprites_red.size(); i++) {
				...
				if (s.energy >= 0) {
					g.drawImage(image_robot_red, (int) (Model.XMAX - 1 - s.x) - 12, (int) (Model.YMAX - 1 - s.y) - 32,
							null);
					...
					g.drawImage(image_broken, (int) (Model.XMAX - 1 - s.x) - 12, (int) (Model.YMAX - 1 - s.y) - 32,
							null);
			}
		}
		...
		private void drawTitles(Graphics g) {
			...
			g.setColor(new Color(128, 0, 0));
			g.drawString(controller.getRedName(), (int) Model.XFLAG_OPPONENT - 80,
					(int) Model.YFLAG_OPPONENT - 2 * FLAG_IMAGE_HEIGHT);
		}
		...
		private void drawFlags(Graphics g) {
			...
			// Red
			g.drawImage(image_flag_red, (int) Model.XFLAG_OPPONENT, (int) Model.YFLAG_OPPONENT - FLAG_IMAGE_HEIGHT,
					null);
			g.setColor(new Color(128, 0, 0));
			...
			if (skipframes > 0)
				...
				if (!controller.update()) {
					...
					if (model.getFlagEnergySelf() < 0.0f && model.getFlagEnergyOpponent() >= 0.0f)
						...
						System.out.println("\nTie.");
					View.this.dispatchEvent(new WindowEvent(View.this, WindowEvent.WINDOW_CLOSING)); // The game is
																										// over, so
																										// close this
																										// window
				}
